...
const buildTaskSystemPrompt = async (taskKey, { variables, lookups = null, reminders = true } = {}) => {
  ...
  if (["jumpForward", "autoJumpForward"].includes(taskKey)) {
    systemPrompt = `${systemPrompt}\n\n[Other Powers' Militaries]\nThe map should not read as though only ${normalizeString(variables.playerPolity) || "the player's polity"} fields any forces. When a major or currently-relevant power (a scenario-defined actor, a country the player has clashed or negotiated with, a power actively at war or mobilizing) plausibly has forces in the field this period - mobilizing, patrolling a border, escorting a fleet, garrisoning a front, reinforcing an ally - reflect it with impacts.unitOps even when nothing dramatic is happening to the player specifically. A brief, minor event (or a line folded into a larger one) is enough to justify it; it does not need its own headline. Keep this proportionate: a country at peace far from any conflict does not need forces conjured for their own sake, and nobody moves against ${normalizeString(variables.playerPolity) || "the player's polity"} out of nothing. But a power whose motive is already on the record - a claim, a grievance, a treaty broken, an ally to protect, an opening left by a weak or distracted neighbour - acts on it, and the player is as legitimate an object of that as any other polity. What is forbidden is inventing hostility the campaign does not support, not hostility itself.`;
  }
