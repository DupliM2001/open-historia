...
//
//   { id, title, body, visibleTo, sourceEventId, createdRound, createdDate,
//     dateline, origin }
//
// visibleTo is a list of polity names, or null for a public document (a
// published treaty, a communiqué). The narrator (the simulation) sees every
// report; a viewer sees those addressed to a polity it speaks for
// (AI/audience.js audienceSeesScoped) — the player's advisor, the player's
// Dossier panel, a leader in a chat, each through the same rule.
//
...
export const normalizeReportOp = (entry) => {
    ...
    if (op === "share" || op === "update") {
        ...
        return { op: "share", reportId, visibleTo };
    }
    return null;
};
...
export const applyReportOps = (reports, ops, { eventId = "", date = "", round = 0, resolvePolity = null } = {}) => {
    ...
    for (const raw of asArray(ops)) {
        ...
        const visibleTo = target.visibleTo === null ? null : [...target.visibleTo, ...known.filter((name) => !target.visibleTo.some((entry) => fold(entry) === fold(name)))];
        next = next.map((report) => (report.id === target.id ? { ...report, visibleTo } : report));
...
export const describeReportsForPrompt = (reports, { sees = null, limit = 12, bodyChars = 160, heading = "[Reports on File]" } = {}) => {
    ...
    const lines = list.map((report) => {
        ...
        return `- ${report.id} · "${report.title}"${report.dateline ? ` (${report.dateline})` : ""} · ${holders}: ${body}`;
    });
    return `${heading}\n${lines.join("\n")}`;
};
...
export const REPORT_VOICE_DIRECTIVE = [
    ...
    "visibleTo lists the polities that hold it, by their full names; leave it empty only for a document genuinely published to all. Widen an existing report with op share and its id when it reaches more hands.",
