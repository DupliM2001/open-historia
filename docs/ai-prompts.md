...
**The user message, for a jump.** `runJumpSegments` builds it as `[<application receipt>, buildSegmentInstruction(…)]`, joined by a blank line. The receipt is `[APPLICATION RESULT FROM YOUR LAST TURN]` (`runtime/applicationReceipt.js`): what the engine dropped, withheld or changed in the previous turn's answer, plus what it applied. It goes in the **user** message, not the system prompt, so the cacheable static prefix is untouched; it is repeated on every segment, because each segment is a separate request; and it is empty on a campaign's first jump, when the message is byte‑for‑byte `buildSegmentInstruction`'s output. See [AI overview](ai-overview.md#the-application-receipt-what-salvage-did-told-to-the-next-turn).
